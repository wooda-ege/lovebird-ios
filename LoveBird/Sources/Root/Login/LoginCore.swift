  ...
  var body: some Reducer<State, Action> {
    Reduce { state, action in
      ...
      case .kakaoLoginTapped(let idToken):
        return .run { send in
          do {
            let kakaoLoginResponse = try await self.apiClient.request(.login(info: AuthRequest.init(provider: SNSProvider.kakao, idToken: idToken))) as LoginResponse
      ...
      case .appleLoginTapped(let auth):
        ...
        case let credential as ASAuthorizationAppleIDCredential:
          ...
          return .run { send in
            do {
              let appleLoginResponse = try await self.apiClient.request(.login(info: AuthRequest.init(provider: SNSProvider.apple, idToken: idToken))) as LoginResponse
